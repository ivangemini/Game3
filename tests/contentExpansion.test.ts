...
describe('content expansion pack', () => {
  it('expands the prototype pools without orphaned combat profiles or recipes', () => {
    expect(PROTOTYPE_BASE_ITEMS).toHaveLength(16);
    expect(PROTOTYPE_FUSION_ITEMS).toHaveLength(12);
    expect(PROTOTYPE_ITEMS).toHaveLength(28);
    expect(PROTOTYPE_FUSION_RECIPES).toHaveLength(12);
    expect(PROTOTYPE_PERKS).toHaveLength(10);
  ...
  it('lets food and chaos energize an adjacent pet laser at the same time', () => {
  ...
  it('activates antenna, slime and reinforced-weapon families through real side contact', () => {
    const antenna = evaluateSynergies(
      inventory(
        placed('radio', 'pocket-radio', 0, 0),
        placed('toaster', 'cursed-toaster', 1, 0),
      ),
      PROTOTYPE_ITEM_MAP,
    );
    ...
    const slime = evaluateSynergies(
      inventory(
        placed('slime', 'slime-can', 0, 0),
        placed('flask', 'poison-flask', 0, 1),
      ),
      PROTOTYPE_ITEM_MAP,
    );
