...
describe('runtime atlas contract', () => {
  it('keeps exactly two portal-facing art atlas requests', () => {
    expect(RUNTIME_ATLASES).toHaveLength(2);
    expect(RUNTIME_ATLASES.map((atlas) => atlas.textureKey)).toEqual([
      ITEM_ATLAS_TEXTURE_KEY,
      PORTRAIT_ATLAS_TEXTURE_KEY,
    ]);
    expect(new Set(RUNTIME_ATLASES.flatMap((atlas) => [atlas.imageUrl, atlas.dataUrl])).size).toBe(4);
  });
  ...
  it('routes stable art keys to the correct atlas without changing frame keys', () => {
    ...
    expect(atlasTextureKeyForArtKey('ui.settings')).toBeNull();
  });
