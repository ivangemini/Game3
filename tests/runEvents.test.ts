...
describe('run events', () => {
  it('expands to nine unique events and keeps every item reward inside known shop content', () => {
    expect(PROTOTYPE_RUN_EVENTS).toHaveLength(9);
    expect(new Set(PROTOTYPE_RUN_EVENTS.map((event) => event.id)).size).toBe(9);
