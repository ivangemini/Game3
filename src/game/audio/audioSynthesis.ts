...
/**
 * Asset-free launch-safe SFX patch mapping.
 *
 * These patches are deliberately short and sparse. Final recorded samples can
 * replace this renderer without changing CombatPanel or the semantic cue API.
 */
export function synthPatchForCue(cue: AudioCue): SynthPatch {
  ...
  switch (cue.id) {
    ...
    case 'item.trigger':
      return patch(tone('square', pitch(430), pitch(520), 0, 58, 0.018));
    case 'item.jammed':
      return patch(tone('sawtooth', pitch(270), pitch(105), 0, 120, 0.026));
    case 'item.slimed':
      return patch(tone('sine', pitch(210), pitch(95), 0, 145, 0.027));
    case 'item.scrambled':
      return patch(
    ...
    case 'item.eclipsed':
      return patch(tone('triangle', pitch(460), pitch(115), 0, 165, 0.026));
    case 'enemy.hit':
      return patch(tone('triangle', pitch(185), pitch(78), 0, 78, 0.036));
    case 'enemy.poison-tick':
      return patch(tone('sine', pitch(305), pitch(218), 0, 96, 0.018));
    case 'poison.apply':
      return patch(
    ...
    case 'shield.gain':
      return patch(tone('sine', pitch(330), pitch(710), 0, 155, 0.028));
    case 'player.hit':
      return patch(
