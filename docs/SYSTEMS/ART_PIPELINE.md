...
## UI atlas contract
- Phaser texture key: `junk-ui`
- Stable frame keys: `ui.<id>`
- Current authored set: daily, archive, trophies, help, settings, reset, coin, fusion, pocket and logo mark.
- Top HUD buttons use atlas icons without changing their tested interactive hitboxes or responsive placements.
- Standalone `public/assets/art/ui/*.svg` sources remain the development/recovery fallback.

## Source → build → runtime
1. Maintain reviewed SVG sources under `public/assets/art/items|heroes|bosses|ui/`.
...
7. `AssetPreloadScene` loads the three atlas pages before gameplay.
8. Verify backpack, shop, fusion, hero-select, boss and HUD contexts before release.
...
- source SVG payload: **53.0 KiB**;
- generated atlas+JSON payload: **94.7 KiB**;
- estimated RGBA texture memory: **10.39 MiB**;
- authored-art request count: **80 standalone source files reduced to 3 atlas requests**.
...
Fallback is resilience, not the normal shipping path for the current authored catalog. Browser QA asserts that a normal production boot does not request the 80 standalone authored files.
...
Core UI symbols use `junk-ui`. Store/thumbnail art remains a separate marketing composition generated from editable sources through `npm run assets:store`; it is not a screenshot of gameplay UI. See `docs/SYSTEMS/STORE_ART.md`.
